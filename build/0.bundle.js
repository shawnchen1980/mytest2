...
var LoginData = _backbone2.default.Model.extend({
	defaults: {
		"email": "",
		"token": ""
	}
...
var LoginView = _backbone2.default.View.extend({
	...
	template: _underscore2.default.template((0, _jquery2.default)('#login-box').html()),
	...
	signin: function signin() {
		...
		_axios2.default.post("http://localhost:3030/signin", { email: this.$email.val().trim(), password: this.$pass.val().trim() }).then(function (res) {
			localStorage.setItem("data", JSON.stringify(res.data));_this.model.set({ email: res.data.email, iat: "" });
		}, function (err) {
			console.log(err);
		});
	}
});
var login = new LoginView({ model: new LoginData() });
